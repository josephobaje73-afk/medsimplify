...
class AITranslator:
    ...
    def simplify(self, text: str, section_name: str) -> str:
        ...
        # FIX: renamed to request_payload so it doesn't get clobbered by
        # the parsed JSON response below (they were both called `payload`,
        # which was confusing and risked reusing the wrong value on retry).
...
def main() -> None:
    ...
    with cast(_StreamlitSpinnerFactoryAPI, cast(object, st)).spinner(
        ...
        if missing:
            # FIX: was a bare `st.info(...)` call outside the type-cast
            # pattern the rest of the file uses; also, `st` may be the
            # `_StreamlitFallback` shim above whose `__getattr__` returns
    ...
    _ = cast(Callable[[str], object], getattr(cast(object, st), "subheader"))(
        f"{(medication.generic_name or drug_name).title()}"
    )
